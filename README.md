...
### 3. SFU 방식은 무엇입니까?
Mesh/P2P 처럼 클라이언트와 클라이언트간의 통신이 이루어 질 경우 각 클라이언트에서 모든 스트림을 처리하는 방식이 아닌 종단 간 미디어 트래픽을 중계하는 중앙 서버를 두어 서버가 스트림 데이터를 처리하는 방식입니다. 각각의 클라이언트들은 필요한 스트림만 받아 디코딩 후 렌더링합니다.
## 개발 환경
